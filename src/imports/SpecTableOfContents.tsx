import svgPaths from "./svg-7fh1tzdyam";
import imgVector from "figma:asset/a553e5bc52e1ba8f039087a006ce0aa826ae132b.png";

function Frame9() {
  return (
    <div className="content-stretch flex gap-[8px] items-center relative shrink-0">
      <div className="flex flex-col font-['Inter:Medium',sans-serif] font-medium justify-center leading-[0] not-italic relative shrink-0 text-[#111827] text-[14px] whitespace-nowrap">
        <p className="leading-[20px]">Table of Contents</p>
      </div>
    </div>
  );
}

function HorizontalBorder() {
  return (
    <div className="bg-[#fbfbfb] h-[56px] relative rounded-tl-[12px] shrink-0 w-full" data-name="HorizontalBorder">
      <div aria-hidden="true" className="absolute border-[#e5e5e5] border-b border-l border-solid border-t inset-0 pointer-events-none rounded-tl-[12px]" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between px-[16px] py-[12px] relative size-full">
          <Frame9 />
        </div>
      </div>
    </div>
  );
}

function Svg() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg />
    </div>
  );
}

function Button() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">Core Conversational AI</p>
      </div>
    </div>
  );
}

function Svg1() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection1() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg1 />
    </div>
  );
}

function Button1() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">Safety and Crisis Man…</p>
      </div>
    </div>
  );
}

function Svg2() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection2() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg2 />
    </div>
  );
}

function Button2() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">User Tracking and Pro…</p>
      </div>
    </div>
  );
}

function Svg3() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection3() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg3 />
    </div>
  );
}

function Button3() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">Content and Psychoed…</p>
      </div>
    </div>
  );
}

function Svg4() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection4() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg4 />
    </div>
  );
}

function Button4() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">Technical Platform Re…</p>
      </div>
    </div>
  );
}

function Svg5() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection5() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg5 />
    </div>
  );
}

function Button5() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">User Access and Auth…</p>
      </div>
    </div>
  );
}

function Svg6() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection6() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg6 />
    </div>
  );
}

function Button6() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">Non-Functional Requir…</p>
      </div>
    </div>
  );
}

function Svg7() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection7() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg7 />
    </div>
  );
}

function Button7() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">Performance Require…</p>
      </div>
    </div>
  );
}

function Svg8() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection8() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg8 />
    </div>
  );
}

function Button8() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">Scalability Requireme…</p>
      </div>
    </div>
  );
}

function Svg9() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection9() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg9 />
    </div>
  );
}

function Button9() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">{`Security & Privacy Req…`}</p>
      </div>
    </div>
  );
}

function Svg10() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection10() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg10 />
    </div>
  );
}

function Button10() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">{`Reliability & Availability`}</p>
      </div>
    </div>
  );
}

function Svg11() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection11() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg11 />
    </div>
  );
}

function Button11() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">{`Usability & Accessibility`}</p>
      </div>
    </div>
  );
}

function Svg12() {
  return (
    <div className="absolute left-[2px] size-[16px] top-[2px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d="M4 6L8 10L12 6" id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonCollapseSection12() {
  return (
    <div className="h-[20px] relative rounded-[4px] shrink-0 w-full" data-name="Button - Collapse section">
      <Svg12 />
    </div>
  );
}

function Button12() {
  return (
    <div className="h-[20px] overflow-clip relative shrink-0 w-full" data-name="Button">
      <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] left-0 not-italic overflow-hidden text-[#171717] text-[14px] text-ellipsis top-[calc(50%-0.5px)] whitespace-nowrap">
        <p className="leading-[20px] overflow-hidden">{`Maintainability & Oper…`}</p>
      </div>
    </div>
  );
}

function Container1() {
  return (
    <div className="h-[1558px] relative shrink-0 w-full" data-name="Container">
      <div className="overflow-auto size-full">
        <div className="content-stretch flex flex-col gap-[20px] items-start p-[24px] relative size-full">
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Executive Summary</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">What Makes This Special</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Project Classification</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Success Criteria</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Product Scope</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">User Journeys</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Journey 1: Sarah - Breaking the Work Overwhelm Spiral</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Journey 2: Marcus - Reclaiming Control When Life Feels Chaotic</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Journey 3: Aisha - Finding Light in the Darkness of Hopelessness</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Journey Requirements Summary</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Domain Requirements</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Regulatory Compliance</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Data Privacy Requirements</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Safety and Clinical Standards</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Deferred Requirements</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Current Requirements</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Technical Foundation</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Project Type</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Platform Requirements</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Technical Constraints</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Integration Requirements</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Scope Definition</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">MVP Features (Must H…</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Growth Features (Sho…</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Vision Features (Could…</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Explicitly Out of Scope…</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">MVP Success Criteria</p>
          </div>
          <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] not-italic overflow-hidden relative shrink-0 text-[#171717] text-[14px] text-ellipsis w-full whitespace-nowrap">
            <p className="leading-[20px] overflow-hidden">Functional Requireme…</p>
          </div>
          <ButtonCollapseSection />
          <Button />
          <ButtonCollapseSection1 />
          <Button1 />
          <ButtonCollapseSection2 />
          <Button2 />
          <ButtonCollapseSection3 />
          <Button3 />
          <ButtonCollapseSection4 />
          <Button4 />
          <ButtonCollapseSection5 />
          <Button5 />
          <ButtonCollapseSection6 />
          <Button6 />
          <ButtonCollapseSection7 />
          <Button7 />
          <ButtonCollapseSection8 />
          <Button8 />
          <ButtonCollapseSection9 />
          <Button9 />
          <ButtonCollapseSection10 />
          <Button10 />
          <ButtonCollapseSection11 />
          <Button11 />
          <ButtonCollapseSection12 />
          <Button12 />
        </div>
      </div>
      <div aria-hidden="true" className="absolute border-[#e5e5e5] border-b border-l border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function TableOfContents() {
  return (
    <div className="bg-white content-stretch flex flex-col items-start relative shrink-0 w-[256px]" data-name="Table of Contents">
      <HorizontalBorder />
      <Container1 />
    </div>
  );
}

function Frame7() {
  return (
    <div className="content-stretch flex items-center p-[2px] relative rounded-[4px] shrink-0">
      <div className="overflow-clip relative shrink-0 size-[16px]" data-name="icon">
        <div className="absolute inset-[4.17%]" data-name="Vector (Stroke)">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14.6667 14.6667">
            <path d={svgPaths.p2b0e0000} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
          </svg>
        </div>
        <div className="absolute inset-[45.83%_45.83%_29.17%_45.83%]" data-name="Vector (Stroke)">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 1.33333 4">
            <path d={svgPaths.p1843a300} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
          </svg>
        </div>
        <div className="absolute inset-[29.17%_45.79%_62.5%_45.83%]" data-name="Vector (Stroke)">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 1.33984 1.33333">
            <path d={svgPaths.p883c00} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Svg13() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[16px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d={svgPaths.p24f3a900} id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d={svgPaths.p1152ef00} id="Vector_2" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d={svgPaths.pb149c00} id="Vector_3" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonMenuDocumentActions() {
  return (
    <div className="relative rounded-[6px] shrink-0 size-[32px]" data-name="Button menu - Document actions">
      <Svg13 />
    </div>
  );
}

function Frame8() {
  return (
    <div className="content-stretch flex gap-[6px] items-center relative shrink-0">
      <Frame7 />
      <ButtonMenuDocumentActions />
    </div>
  );
}

function Frame11() {
  return (
    <div className="bg-[#fbfbfb] relative rounded-tr-[12px] shrink-0 w-full">
      <div aria-hidden="true" className="absolute border border-[#e7e7e7] border-solid inset-0 pointer-events-none rounded-tr-[12px]" />
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between px-[20px] py-[12px] relative w-full">
          <div className="overflow-clip relative shrink-0 size-[18px]" data-name="icon">
            <div className="absolute inset-[8.33%]" data-name="Vector (Stroke)">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 15">
                <path d={svgPaths.p16a2e8f0} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
              </svg>
            </div>
            <div className="absolute inset-[8.33%_58.33%_8.33%_33.33%]" data-name="Vector (Stroke)">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 1.5 15">
                <path d={svgPaths.p38fd8980} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
              </svg>
            </div>
            <div className="absolute bottom-[33.33%] left-1/2 right-[29.17%] top-[33.33%]" data-name="Vector (Stroke)">
              <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 3.74989 5.99989">
                <path d={svgPaths.p96f6800} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
              </svg>
            </div>
          </div>
          <Frame8 />
        </div>
      </div>
    </div>
  );
}

function Paragraph() {
  return (
    <div className="relative shrink-0 w-full" data-name="Paragraph">
      <div aria-hidden="true" className="absolute border-[#e7e7e7] border-b border-l border-r border-solid inset-0 pointer-events-none" />
      <div className="content-stretch flex flex-col gap-[40px] items-start not-italic p-[28px] relative text-black w-full">
        <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold h-[36px] justify-center leading-[0] relative shrink-0 text-[30px] w-[288.452px]">
          <p className="leading-[40px] whitespace-pre-wrap">Executive Summary</p>
        </div>
        <p className="font-['Inter:Regular',sans-serif] font-normal leading-[32px] min-w-full relative shrink-0 text-[18px] w-[min-content] whitespace-pre-wrap" style={{ fontFeatureSettings: "'kern' 0" }}>{`The Plus Minus Clinic App is a custom unified digital platform built exclusively for Plus Minus Clinic, a Sydney-based skin and aesthetics clinic. This bespoke solution addresses critical operational fragmentation by serving as their centralized hub that combines CRM, content delivery, and commerce capabilities. Acting as Plus Minus Clinic's private "Instagram + WeChat + booking system," the platform eliminates dependency on third-party platforms while streamlining operations for both clinic staff and clients. The urgency is driven by September 2025 advertising restrictions that will eliminate the clinic's primary marketing channels, making this owned digital presence essential for business continuity. The solution replaces multiple disconnected systems with a single platform that handles all client touchpoints, from content consumption to booking to service delivery, while providing complete ownership and control over their digital presence.`}</p>
        <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold h-[36px] justify-center leading-[0] relative shrink-0 text-[30px] w-[364.212px]">
          <p className="leading-[40px] whitespace-pre-wrap">What Makes This Special</p>
        </div>
        <p className="font-['Inter:Regular',sans-serif] font-normal leading-[32px] min-w-full relative shrink-0 text-[18px] w-[min-content] whitespace-pre-wrap" style={{ fontFeatureSettings: "'kern' 0" }}>{`Unlike generic clinic management software or standard booking platforms, the Plus Minus Clinic App is custom-built to solve Plus Minus Clinic's specific operational challenges and regulatory constraints. The primary differentiator is the bespoke nature - it's designed around their exact workflows, client journey, and business model rather than forcing them to adapt to a one-size-fits-all solution. This approach ensures perfect alignment with their September 2025 regulatory challenge while integrating seamlessly with their current processes, eliminating unnecessary features or forced workflows common in generic platforms.`}</p>
      </div>
    </div>
  );
}

function Frame12() {
  return (
    <div className="content-stretch flex flex-[1_0_0] flex-col items-start min-h-px min-w-px relative">
      <Frame11 />
      <Paragraph />
    </div>
  );
}

function VerticalBorder() {
  return (
    <div className="absolute h-[845px] left-0 top-[-1px] w-[861px]" data-name="VerticalBorder">
      <div className="content-stretch flex items-start overflow-clip px-[12px] py-[16px] relative rounded-[inherit] size-full">
        <TableOfContents />
        <Frame12 />
      </div>
      <div aria-hidden="true" className="absolute border-[#ccc] border-r border-solid inset-0 pointer-events-none" />
    </div>
  );
}

function Overlay() {
  return (
    <div className="bg-[#f2f2f2] content-stretch flex flex-col items-center justify-center p-[10px] relative rounded-[9999px] shrink-0" data-name="Overlay">
      <div className="overflow-clip relative shrink-0 size-[20px]" data-name="icon">
        <div className="absolute inset-[4.17%_12.5%]" data-name="Vector (Stroke)">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 15 18.3333">
            <path d={svgPaths.p172ac800} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
          </svg>
        </div>
        <div className="absolute inset-[4.17%_12.5%_62.5%_54.17%]" data-name="Vector (Stroke)">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 6.66667 6.66667">
            <path d={svgPaths.p3ff89c00} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
          </svg>
        </div>
        <div className="absolute bottom-[41.67%] left-[29.17%] right-[29.17%] top-1/2" data-name="Vector (Stroke)">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 8.33333 1.66667">
            <path d={svgPaths.p148e2b40} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
          </svg>
        </div>
        <div className="absolute bottom-1/4 left-[29.17%] right-[29.17%] top-[66.67%]" data-name="Vector (Stroke)">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 8.33333 1.66667">
            <path d={svgPaths.p148e2b40} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
          </svg>
        </div>
        <div className="absolute inset-[33.33%_54.17%_58.33%_29.17%]" data-name="Vector (Stroke)">
          <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 3.33333 1.66667">
            <path d={svgPaths.p19066100} fill="var(--fill-0, #737373)" id="Vector (Stroke)" />
          </svg>
        </div>
      </div>
    </div>
  );
}

function Frame4() {
  return (
    <div className="content-stretch flex font-['Inter:Semi_Bold',sans-serif] font-semibold gap-[12px] items-center relative shrink-0">
      <p className="relative shrink-0 text-[16px] text-black">Product Manager</p>
      <p className="relative shrink-0 text-[#737373] text-[14px]">“John”</p>
    </div>
  );
}

function Container3() {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start leading-[1.5] not-italic relative shrink-0" data-name="Container">
      <Frame4 />
      <p className="font-['Inter:Regular',sans-serif] font-normal relative shrink-0 text-[14px] text-black">Build a PRD</p>
    </div>
  );
}

function Frame3() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex gap-[12px] items-center relative w-full">
        <Overlay />
        <Container3 />
      </div>
    </div>
  );
}

function AgentSelector() {
  return (
    <div className="bg-white max-w-[700px] relative rounded-[8px] shrink-0 w-full" data-name="AgentSelector">
      <div aria-hidden="true" className="absolute border border-[#ccc] border-solid inset-0 pointer-events-none rounded-[8px]" />
      <div className="flex flex-row items-center max-w-[inherit] size-full">
        <div className="content-stretch flex items-center justify-between max-w-[inherit] p-[13px] relative w-full">
          <Frame3 />
          <div className="h-[14px] relative shrink-0 w-[20px]" data-name="icon">
            <div className="bg-clip-padding border-0 border-[transparent] border-solid relative size-full">
              <div className="absolute inset-[31.25%_10.53%_25.54%_10.54%]" data-name="Vector (Stroke)">
                <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 11.0501 6.04973">
                  <path d={svgPaths.p1da4c280} fill="var(--fill-0, black)" id="Vector (Stroke)" />
                </svg>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

function Container2() {
  return (
    <div className="bg-white relative shrink-0 w-full" data-name="Container">
      <div aria-hidden="true" className="absolute border-[#ccc] border-b border-solid inset-0 pointer-events-none" />
      <div className="flex flex-col justify-center size-full">
        <div className="content-stretch flex flex-col gap-[8px] items-start justify-center p-[16px] relative w-full">
          <p className="font-['Inter:Semi_Bold',sans-serif] font-semibold leading-[1.5] not-italic relative shrink-0 text-[#737373] text-[14px]">Select Agent:</p>
          <AgentSelector />
        </div>
      </div>
    </div>
  );
}

function Frame6() {
  return (
    <div className="content-stretch flex flex-col gap-[10px] h-[140px] items-start relative shrink-0 w-full">
      <Container2 />
    </div>
  );
}

function Svg14() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[14px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14">
        <g id="SVG">
          <path d={svgPaths.p2c6800c0} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.16667" />
        </g>
      </svg>
    </div>
  );
}

function Background() {
  return (
    <div className="bg-[#d20f1f] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <Svg14 />
    </div>
  );
}

function Svg15() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[14px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14">
        <g id="SVG">
          <path d={svgPaths.p2c6800c0} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.16667" />
        </g>
      </svg>
    </div>
  );
}

function Background1() {
  return (
    <div className="bg-[#d20f1f] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <Svg15 />
    </div>
  );
}

function Svg16() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[14px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14">
        <g id="SVG">
          <path d={svgPaths.p2c6800c0} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.16667" />
        </g>
      </svg>
    </div>
  );
}

function Background2() {
  return (
    <div className="bg-[#d20f1f] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <Svg16 />
    </div>
  );
}

function Svg17() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[14px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14">
        <g id="SVG">
          <path d={svgPaths.p2c6800c0} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.16667" />
        </g>
      </svg>
    </div>
  );
}

function Background3() {
  return (
    <div className="bg-[#d20f1f] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <Svg17 />
    </div>
  );
}

function Svg18() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[14px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14">
        <g id="SVG">
          <path d={svgPaths.p2c6800c0} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.16667" />
        </g>
      </svg>
    </div>
  );
}

function Background4() {
  return (
    <div className="bg-[#d20f1f] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <Svg18 />
    </div>
  );
}

function Svg19() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[14px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 14 14">
        <g id="SVG">
          <path d={svgPaths.p2c6800c0} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.16667" />
        </g>
      </svg>
    </div>
  );
}

function Background5() {
  return (
    <div className="bg-[#d20f1f] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <Svg19 />
    </div>
  );
}

function Background6() {
  return (
    <div className="bg-[rgba(210,15,31,0.8)] overflow-clip relative rounded-[9999px] shadow-[0px_0px_0px_2px_rgba(210,15,31,0.3)] shrink-0 size-[24px]" data-name="Background">
      <div className="-translate-x-1/2 -translate-y-1/2 absolute flex flex-col font-['Inter:Medium',sans-serif] font-medium h-[14.5px] justify-center leading-[0] left-[calc(50%+0.17px)] not-italic text-[12px] text-center text-white top-[calc(50%-0.25px)] w-[7.207px]">
        <p className="leading-[16px] whitespace-pre-wrap">7</p>
      </div>
    </div>
  );
}

function Background7() {
  return (
    <div className="bg-[#f2f2f2] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <div className="-translate-x-1/2 -translate-y-1/2 absolute flex flex-col font-['Inter:Medium',sans-serif] font-medium h-[14.5px] justify-center leading-[0] left-[calc(50%+0.18px)] not-italic text-[#737373] text-[12px] text-center top-[calc(50%-0.25px)] w-[7.924px]">
        <p className="leading-[16px] whitespace-pre-wrap">8</p>
      </div>
    </div>
  );
}

function Background8() {
  return (
    <div className="bg-[rgba(242,242,242,0.8)] overflow-clip relative rounded-[9999px] shadow-[0px_0px_0px_2px_rgba(210,15,31,0.3)] shrink-0 size-[24px]" data-name="Background">
      <div className="-translate-x-1/2 -translate-y-1/2 absolute flex flex-col font-['Inter:Medium',sans-serif] font-medium h-[14.5px] justify-center leading-[0] left-[calc(50%+0.18px)] not-italic text-[#737373] text-[12px] text-center top-[calc(50%-0.25px)] w-[7.914px]">
        <p className="leading-[16px] whitespace-pre-wrap">9</p>
      </div>
    </div>
  );
}

function Background9() {
  return (
    <div className="bg-[#f2f2f2] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <div className="-translate-x-1/2 -translate-y-1/2 absolute flex flex-col font-['Inter:Medium',sans-serif] font-medium h-[14.5px] justify-center leading-[0] left-[calc(50%+0.39px)] not-italic text-[#737373] text-[12px] text-center top-[calc(50%-0.25px)] w-[13.517px]">
        <p className="leading-[16px] whitespace-pre-wrap">10</p>
      </div>
    </div>
  );
}

function Background10() {
  return (
    <div className="bg-[#f2f2f2] relative rounded-[9999px] shrink-0 size-[24px]" data-name="Background">
      <div className="-translate-x-1/2 -translate-y-1/2 absolute flex flex-col font-['Inter:Medium',sans-serif] font-medium h-[14.5px] justify-center leading-[0] left-[calc(50%+0.72px)] not-italic text-[#737373] text-[12px] text-center top-[calc(50%-0.25px)] w-[11.392px]">
        <p className="leading-[16px] whitespace-pre-wrap">11</p>
      </div>
    </div>
  );
}

function Frame5() {
  return (
    <div className="content-stretch flex gap-[8px] items-center relative shrink-0 w-full">
      <Background />
      <Background1 />
      <Background2 />
      <Background3 />
      <Background4 />
      <Background5 />
      <Background6 />
      <Background7 />
      <Background8 />
      <Background9 />
      <Background10 />
    </div>
  );
}

function OverlayHorizontalBorder() {
  return (
    <div className="bg-[rgba(242,242,242,0.3)] relative shrink-0 w-full" data-name="Overlay+HorizontalBorder">
      <div aria-hidden="true" className="absolute border-[#ccc] border-b border-solid inset-0 pointer-events-none" />
      <div className="content-stretch flex flex-col gap-[8px] items-start px-[16px] py-[12px] relative w-full">
        <div className="flex flex-col font-['Inter:Medium',sans-serif] font-medium justify-center leading-[0] not-italic relative shrink-0 text-[14px] text-black w-full">
          <p className="leading-[20px] whitespace-pre-wrap">Step 7 of 11: Project Type</p>
        </div>
        <Frame5 />
      </div>
    </div>
  );
}

function Container6() {
  return (
    <div className="bg-white relative rounded-[10px] shrink-0 w-full" data-name="Container">
      <div aria-hidden="true" className="absolute border border-[#e5e5e5] border-solid inset-0 pointer-events-none rounded-[10px]" />
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex flex-col gap-[12px] items-start not-italic p-[12px] relative text-[14px] w-full">
        <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal justify-center leading-[0] relative shrink-0 text-[#374151] w-full">
          <p className="leading-[20px] whitespace-pre-wrap">{`Based on your Product Brief [1], I can see you've laid out a comprehensive vision for MindSpace. Let me extract what I understand:`}</p>
        </div>
        <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#111827] w-full">
          <p className="whitespace-pre-wrap">
            <span className="leading-[20px]">{`What you're building:`}</span>
            <span className="font-['Inter:Regular',sans-serif] font-normal leading-[20px] not-italic text-[#374151]">{` An AI-powered mental health app that provides daily emotional support through evidence-based practices, designed to intercept users at trigger moments and prevent them from falling into "functional freeze" states.`}</span>
          </p>
        </div>
        <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#111827] w-full">
          <p className="whitespace-pre-wrap">
            <span className="leading-[20px]">Problem it solves:</span>
            <span className="font-['Inter:Regular',sans-serif] font-normal leading-[20px] not-italic text-[#374151]">{` Addresses the critical gap where people needing daily mental health support turn to unregulated, unreliable apps that may provide harmful guidance during vulnerable moments, perpetuating disconnection from their lives rather than providing meaningful help.`}</span>
          </p>
        </div>
        <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#111827] w-full">
          <p className="whitespace-pre-wrap">
            <span className="leading-[20px]">Target users:</span>
            <span className="font-['Inter:Regular',sans-serif] font-normal leading-[20px] not-italic text-[#374151]">{` Tech-savvy Gen Z and Millennial adults who are proactive about mental health - specifically those on therapy waitlists or between sessions who need ongoing daily guidance and are open to AI-powered interactions.`}</span>
          </p>
        </div>
        <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[#111827] w-full">
          <p className="whitespace-pre-wrap">
            <span className="leading-[20px]">What makes it special:</span>
            <span className="font-['Inter:Regular',sans-serif] font-normal leading-[20px] not-italic text-[#374151]">{` Your proprietary empathy engine trained on crisis worker data, evidence-based foundation with consulting psychologist oversight, and the focus on preventing functional freeze states rather than just general wellness.`}</span>
          </p>
        </div>
        <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal justify-center leading-[20px] relative shrink-0 text-[#374151] w-full whitespace-pre-wrap">
          <p className="mb-0">How does this align with your vision? Should we refine any of these points</p>
          <p>before we move forward with the PRD structure?</p>
        </div>
        <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal justify-center leading-[20px] relative shrink-0 text-[#374151] w-full whitespace-pre-wrap">
          <p className="mb-0">{`Also, I want to understand - what's driving the urgency around this project`}</p>
          <p className="mb-0">{`right now? Is there a specific timeline or market opportunity you're`}</p>
          <p>responding to?</p>
        </div>
      </div>
    </div>
  );
}

function Container5() {
  return (
    <div className="absolute content-stretch flex flex-col gap-[8px] items-start left-[20px] pr-[32px] top-[-300px] w-[483px]" data-name="Container">
      <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal h-[16px] justify-center leading-[0] not-italic relative shrink-0 text-[#737373] text-[12px] w-[98.412px]">
        <p className="leading-[16px] whitespace-pre-wrap">Product Manager</p>
      </div>
      <Container6 />
      <p className="font-['Inter:Regular',sans-serif] font-normal leading-[16px] not-italic relative shrink-0 text-[#737373] text-[12px]">16 Jan 2026, 17:17</p>
    </div>
  );
}

function Container8() {
  return (
    <div className="bg-[#541b1f] relative rounded-[10px] shrink-0" data-name="Container">
      <div aria-hidden="true" className="absolute border border-[#e5e5e5] border-solid inset-0 pointer-events-none rounded-[10px]" />
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex flex-col items-start p-[12px] relative">
        <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal justify-center leading-[0] not-italic relative shrink-0 text-[14px] text-white w-full">
          <p className="leading-[20px] whitespace-pre-wrap">What is the current status of this project?</p>
        </div>
      </div>
    </div>
  );
}

function Container7() {
  return (
    <div className="absolute content-stretch flex flex-col gap-[8px] items-end left-[20px] pl-[32px] top-[416px] w-[483px]" data-name="Container">
      <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal justify-center leading-[0] not-italic relative shrink-0 text-[#737373] text-[12px] whitespace-nowrap">
        <p className="leading-[16px]">Me</p>
      </div>
      <Container8 />
      <p className="font-['Inter:Regular',sans-serif] font-normal leading-[16px] not-italic relative shrink-0 text-[#737373] text-[12px]">25 Feb 2026, 12:08</p>
    </div>
  );
}

function Container4() {
  return (
    <div className="flex-[1_0_0] min-h-px min-w-px relative w-full" data-name="Container">
      <div className="overflow-auto size-full">
        <div className="content-stretch flex flex-col gap-[125px] items-start px-[20px] relative size-full">
          <Container5 />
          <Container7 />
        </div>
      </div>
    </div>
  );
}

function TextareaChatMessageInput() {
  return (
    <div className="bg-white flex-[1_0_0] h-[40px] min-h-px min-w-px relative rounded-[8px]" data-name="Textarea - Chat message input">
      <div className="overflow-auto relative size-full">
        <div className="-translate-y-1/2 absolute flex flex-col font-['Inter:Regular',sans-serif] font-normal h-[20px] justify-center leading-[0] left-[13px] not-italic text-[#737373] text-[14px] top-[19px] w-[118.513px]">
          <p className="leading-[20px] whitespace-pre-wrap">Type something...</p>
        </div>
      </div>
      <div aria-hidden="true" className="absolute border border-[#ccc] border-solid inset-0 pointer-events-none rounded-[8px]" />
    </div>
  );
}

function Svg20() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[16px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d={svgPaths.p1b0900} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M8 12.6663V3.33301" id="Vector_2" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonSendMessage() {
  return (
    <div className="bg-[#d1d5db] overflow-clip relative rounded-[9999px] shadow-[0px_1px_3px_0px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)] shrink-0 size-[40px]" data-name="Button - Send message">
      <Svg20 />
    </div>
  );
}

function BackgroundHorizontalBorder() {
  return (
    <div className="bg-white relative shrink-0 w-full" data-name="Background+HorizontalBorder">
      <div aria-hidden="true" className="absolute border-[#ccc] border-solid border-t inset-0 pointer-events-none" />
      <div className="content-stretch flex gap-[12px] items-start p-[16px] relative w-full">
        <TextareaChatMessageInput />
        <ButtonSendMessage />
      </div>
    </div>
  );
}

function ComplementaryAiChatAssistant() {
  return (
    <div className="absolute bg-white content-stretch flex flex-col h-[845px] items-start left-[861px] top-[-1px] w-[523px]" data-name="Complementary - AI Chat Assistant">
      <Frame6 />
      <OverlayHorizontalBorder />
      <Container4 />
      <BackgroundHorizontalBorder />
    </div>
  );
}

function Container() {
  return (
    <div className="absolute h-[844px] left-[56px] right-0 top-[56px]" data-name="Container">
      <VerticalBorder />
      <ComplementaryAiChatAssistant />
    </div>
  );
}

function LogoTransparentSvg() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 overflow-clip size-[32px] top-1/2" data-name="logo_transparent.svg">
      <div className="absolute inset-[6.32%]" data-name="Vector">
        <img alt="" className="absolute block max-w-none size-full" height="27.953" src={imgVector} width="27.953" />
      </div>
    </div>
  );
}

function LogoTransparentSvgFill() {
  return (
    <div className="absolute left-0 overflow-clip size-[32px] top-0" data-name="logo_transparent.svg fill">
      <LogoTransparentSvg />
    </div>
  );
}

function SprynLogo() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 overflow-clip size-[32px] top-[calc(50%-418px)]" data-name="Spryn logo">
      <LogoTransparentSvgFill />
    </div>
  );
}

function Frame() {
  return (
    <div className="absolute inset-[20%]" data-name="Frame">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24">
        <g id="Frame">
          <path d={svgPaths.p3ce5e700} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" />
        </g>
      </svg>
    </div>
  );
}

function Projects() {
  return (
    <div className="h-[40px] relative rounded-[6px] shrink-0 w-full" data-name="Projects">
      <Frame />
    </div>
  );
}

function Svg21() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[24px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24">
        <g id="SVG">
          <path d={svgPaths.pc0a4f21} id="Vector" stroke="var(--stroke-0, white)" strokeLinejoin="round" strokeWidth="1.5" />
        </g>
      </svg>
    </div>
  );
}

function Artifacts() {
  return (
    <div className="relative rounded-[6px] shrink-0 size-[40px]" data-name="Artifacts">
      <Svg21 />
    </div>
  );
}

function Frame1() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[24px] top-1/2" data-name="Frame">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 24 24">
        <g id="Frame">
          <path d={svgPaths.p1beb9e00} id="Vector" stroke="var(--stroke-0, white)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
        </g>
      </svg>
    </div>
  );
}

function Projects1() {
  return (
    <div className="relative rounded-[6px] shrink-0 size-[40px]" data-name="Projects">
      <Frame1 />
    </div>
  );
}

function Frame13() {
  return (
    <div className="absolute content-stretch flex flex-col gap-[12px] items-start left-[8px] top-[64px] w-[40px]">
      <Projects />
      <Artifacts />
      <Projects1 />
    </div>
  );
}

function Sidebar() {
  return (
    <div className="absolute bg-[#541b1f] h-[900px] left-0 top-0 w-[56px]" data-name="Sidebar">
      <SprynLogo />
      <Frame13 />
    </div>
  );
}

function Frame2() {
  return (
    <div className="relative shrink-0 size-[18px]" data-name="Frame">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 18 18">
        <g id="Frame">
          <path d={svgPaths.p3885cf00} id="Vector" stroke="var(--stroke-0, #6B7280)" strokeLinecap="round" strokeLinejoin="round" />
        </g>
      </svg>
    </div>
  );
}

function Container10() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <div className="flex flex-col font-['Inter:Regular',sans-serif] font-normal justify-center leading-[0] not-italic relative shrink-0 text-[#6b7280] text-[14px] whitespace-nowrap">
        <p className="leading-[20px]">Plus Minus Clinic</p>
      </div>
    </div>
  );
}

function Svg22() {
  return (
    <div className="h-[12px] relative shrink-0 w-[7.5px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 7.5 12">
        <g id="SVG">
          <path d={svgPaths.p2d6f8600} fill="var(--fill-0, #9CA3AF)" id="Vector" />
        </g>
      </svg>
    </div>
  );
}

function Container11() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <Svg22 />
    </div>
  );
}

function Svg23() {
  return (
    <div className="relative shrink-0 size-[16px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d={svgPaths.pa3d9100} id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d={svgPaths.p32b7d180} id="Vector_2" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M10.6667 8.66667H5.33333" id="Vector_3" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M10.6667 11.3333H5.33333" id="Vector_4" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d="M6.66667 6H5.33333" id="Vector_5" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function Button13() {
  return (
    <div className="content-stretch flex gap-[8px] items-center px-[8px] py-[4px] relative rounded-[4px] shrink-0" data-name="Button">
      <Svg23 />
      <div className="flex flex-col font-['Inter:Medium',sans-serif] font-medium justify-center leading-[0] not-italic relative shrink-0 text-[#111827] text-[14px] whitespace-nowrap">
        <p className="leading-[20px]">Product Requirements Document</p>
      </div>
    </div>
  );
}

function Container12() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <Button13 />
    </div>
  );
}

function Container9() {
  return (
    <div className="relative shrink-0" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex gap-[8px] items-center relative">
        <Frame2 />
        <Container10 />
        <Container11 />
        <Container12 />
      </div>
    </div>
  );
}

function Svg24() {
  return (
    <div className="-translate-x-1/2 -translate-y-1/2 absolute left-1/2 size-[16px] top-1/2" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="SVG">
          <path d={svgPaths.p151f3900} id="Vector" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d={svgPaths.pc1efc0} id="Vector_2" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
          <path d={svgPaths.p2d6e5280} id="Vector_3" stroke="var(--stroke-0, #737373)" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
        </g>
      </svg>
    </div>
  );
}

function ButtonMenuDocumentActions1() {
  return (
    <div className="relative rounded-[6px] shrink-0 size-[32px]" data-name="Button menu - Document actions">
      <Svg24 />
    </div>
  );
}

function Svg25() {
  return (
    <div className="h-[12px] relative shrink-0 w-[9px]" data-name="SVG">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 9 12">
        <g id="SVG">
          <path d={svgPaths.pdb2cfc0} fill="var(--fill-0, #2E2E2E)" id="Vector" />
        </g>
      </svg>
    </div>
  );
}

function Container14() {
  return (
    <div className="content-stretch flex flex-col items-start relative shrink-0" data-name="Container">
      <Svg25 />
    </div>
  );
}

function Button14() {
  return (
    <div className="bg-white content-stretch flex gap-[8px] items-center px-[12px] py-[6px] relative rounded-[4px] shrink-0" data-name="Button">
      <div aria-hidden="true" className="absolute border border-[#d10f19] border-solid inset-0 pointer-events-none rounded-[4px]" />
      <Container14 />
      <div className="flex flex-col font-['Inter:Semi_Bold',sans-serif] font-semibold h-[20px] justify-center leading-[0] not-italic relative shrink-0 text-[#2e2e2e] text-[14px] text-center w-[54.02px]">
        <p className="leading-[16px] whitespace-pre-wrap">Preview</p>
      </div>
    </div>
  );
}

function Container13() {
  return (
    <div className="relative shrink-0" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex gap-[12px] items-center relative">
        <ButtonMenuDocumentActions1 />
        <Button14 />
      </div>
    </div>
  );
}

function TopNavigationHeader() {
  return (
    <div className="absolute bg-white content-stretch flex h-[56px] items-center justify-between pb-px px-[16px] right-0 top-0 w-[1384px]" data-name="Top Navigation / Header">
      <div aria-hidden="true" className="absolute border-[#ccc] border-b border-solid inset-0 pointer-events-none" />
      <Container9 />
      <Container13 />
    </div>
  );
}

function Frame10() {
  return (
    <div className="absolute h-[30px] left-[910px] top-[441px] w-[13px]">
      <svg className="absolute block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 13 30">
        <g id="Frame 219199440">
          <rect fill="var(--fill-0, white)" height="29" rx="6" width="12" x="0.5" y="0.5" />
          <rect height="29" rx="6" stroke="var(--stroke-0, #CCCCCC)" width="12" x="0.5" y="0.5" />
          <line id="Line 19" stroke="var(--stroke-0, #CCCCCC)" strokeLinecap="round" x1="2.5" x2="10.5" y1="11.5" y2="11.5" />
          <line id="Line 20" stroke="var(--stroke-0, #CCCCCC)" strokeLinecap="round" x1="2.5" x2="10.5" y1="14.5" y2="14.5" />
          <line id="Line 21" stroke="var(--stroke-0, #CCCCCC)" strokeLinecap="round" x1="2.5" x2="10.5" y1="17.5" y2="17.5" />
        </g>
      </svg>
    </div>
  );
}

export default function SpecTableOfContents() {
  return (
    <div className="bg-white relative size-full" data-name="Spec Table of Contents">
      <Container />
      <Sidebar />
      <TopNavigationHeader />
      <Frame10 />
    </div>
  );
}